...
class Segdata_reader:
    ...
    def labelme_json_to_dataset(
        ...
        Return:
            ...
            img data:
                shape (batch size, img height, img width, no. of color channel).
            label data:
...
class SegDataSequence(Sequence):
    ...
    def __init__(self, img_path=None,
        ...
        if padding:
            self.polyline_thickness = cv.FILLED
        else:
    ...
    def __getitem__(self, idx):
        ...
        def _imgaug_to_array(img, label, pos):
            if isinstance(self.augmenter, iaa.meta.Sequential):
                label = np.expand_dims(label, axis=0)
                img, label = self.augmenter(
                    image=img,
        ...
        def _read_json(_path_list, _pos):
            for i, name in enumerate(_path_list):
                ...
                y = label_data[pos].astype("uint8")

                data_shapes = data["shapes"]
                for data_shape in data_shapes:
                    label_name = data_shape["label"]
                    shape_type = data_shape["shape_type"]
                    ...
                    if shape_type in self.allowed_shape_types:
                        ...
                        if index >= 0:
                            points = np.array(data_shape["points"])
                            points = (points/zoom_r).astype(int)

                            label_im = np.zeros((*self.size, 1), dtype="uint8")

                            if shape_type == "linestrip":
                                cv.polylines(label_im,
                                              [points], False,
                                              (1, 0, 0), self.line_thickness)
                            elif shape_type == "polygon":
                                cv.drawContours(label_im,
                                                 [points], -1,
                                ...
                                raise ValueError(f"Invalid format: {self.shape_type}")
        ...
        def _read_layer(_path_list, _pos):
            for i, name in enumerate(_path_list):
                pos = _pos + i
                try:
                    label = self._img_reader(osp.join(
                        self.label_path,
                        name[:name.rfind(".")] + ".png"))
                except:
                    label = self._img_reader(osp.join(
                ...
                label, _ = self._img_processor(label, self.size)

                img = self._img_reader(os.path.join(self.img_path, name))
                img, _ = self._img_processor(img, self.size)
