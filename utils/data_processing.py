...
class Segdata_reader:
    ...
    def _file_to_array(self, img_path, label_path,
        ...
        path_list = [slice_id[0], slice_id[1]]
    ...
    def labelme_json_to_sequence(
        ...
        seq = SegDataSequence(
            ...
            augmenter=self.augmenter, aug_times=self.aug_times,
            padding=padding, line_thickness=line_thickness,
            shuffle=shuffle, seed=seed,
            classifi_mode=classifi_mode,
            encoding=encoding,
            format="json")
        path_list = self._process_paths(seq.path_list)
        self.file_names = path_list.tolist()
    ...
    def __getitem__(self, idx):
        ...
        def _imgaug_to_array(img, label, pos):
            ...
            if ((pos == 0 and remaind_times == 0)
                ...
                if self.rescale is None:
                    ...
                    train_data[pos] = img*self.rescale
        ...
        def _read_json(_path_list):
            ...
            for name in _path_list:
                ...
                pos = _imgaug_to_array(img, y, pos)

        def _read_layer(_path_list):
            pos = 0
            for name in _path_list:
                try:
                    label = Image.open(os.path.join(
        ...
        if self.format == "json":
            _read_json(path_list)
        elif self.format == "layer":
            _read_layer(path_list)
